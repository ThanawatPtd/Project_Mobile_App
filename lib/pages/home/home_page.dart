              ...
              if (snapshot2.hasData){
                  ...
                  return Column(
                    children: <Widget> [
                      ...
                      SizedBox(height: 20.h,),
                      StreamBuilder(stream: planService.getPlans(), builder: (context3, snapshot3) {
                        if (snapshot3.hasData){
                          var planList = snapshot3.data?.docs ?? [];
                          
                          return Container(child: Column(
                            children: [
                              Padding(padding: EdgeInsets.only(left: 20.h, right: 20.h), child: Align(alignment: Alignment.centerLeft,child: Text("Plan",style: TextStyle(fontSize: 18.h),textAlign: TextAlign.left,),),),
                              CarouselSlider.builder(itemCount: planList.length, itemBuilder: (context, index, realIndex) {
                                DocumentSnapshot doucument = planList[index];
                                Map<String, dynamic> data =
                                    doucument.data() as Map<String, dynamic>;
                                String docId = doucument.id;
                                String name = data['Name'];

                                return showPlan(docId, name, 10);
